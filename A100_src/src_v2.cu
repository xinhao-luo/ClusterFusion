 ...
 */
 __global__ void decode(
    ...
    __align__(16) half embedding_reg[EMBEDDING_TILE_LENGTH], residual_embedding_reg[EMBEDDING_TILE_LENGTH];
    ...
    // #------------------------------------------------------------------------
    {
        ...
        for (int di = 0; di < EMBEDDING_TILE_LENGTH / 8; di++) {
            *(uint4 * )(&residual_embedding_reg[8 * di]) = *(uint4 * )(&input[batch_idx * EMBEDDING_DIM + tid * EMBEDDING_TILE_LENGTH + 8 * di]);
        }
        float local_tmp_float = 0;
        #pragma unroll
        for (int di = 0; di < EMBEDDING_TILE_LENGTH; di++) {
            local_tmp_float += __half2float(residual_embedding_reg[di]) * __half2float(residual_embedding_reg[di]);
        }
        // reduce in block
        #pragma unroll
        ...
        #pragma unroll
        for (int di = 0; di < EMBEDDING_TILE_LENGTH; di++) {
            embedding_reg[di] = __hmul(__hmul(residual_embedding_reg[di], local_tmp_float), weight_reg[di]);
        }
    }
    // #------------------------------------------------------------------------
    ...
    for (int i = 0; i < HEAD_DIM; i++) {
        ...
        for (int j = 0; j < EMBEDDING_TILE_LENGTH; ++j) {
            local_tmp_float += __half2float(embedding_reg[j]) * __half2float(weight_reg[j]);
        }
        // warp level reduce
        #pragma unroll
        for (int stride = 16; stride > 0; stride >>= 1) {
        ...
        if (lane_id == 0) {
            block_reduce_half[i * (BLOCK_SIZE / WARP_SIZE) + warp_id] = __float2half(local_tmp_float);
        }
    }
    ...
    for (int i = 0; i < HEAD_DIM; i++) {
        ...
        if (lane_id == 0) {
            block_reduce_half[HEAD_DIM * (BLOCK_SIZE / WARP_SIZE) + i * (BLOCK_SIZE / WARP_SIZE) + warp_id] = local_tmp_float;
        }
    }
    __syncthreads();
    ...
    if (tid < HEAD_DIM) {
        ...
        #pragma unroll
        for (int di = 0; di < BLOCK_SIZE / WARP_SIZE / 8; di++) {
            *(uint4 * )(&local_half2[8 * di]) = *(uint4 * )(&block_reduce_half[type * HEAD_DIM * (BLOCK_SIZE / WARP_SIZE) + (position + HEAD_DIM / 2) * (BLOCK_SIZE / WARP_SIZE) + 8 * di]);
        }
    ...
    for (int i = 0; i < HEAD_DIM; i++){
        ...
        if (lane_id == 0) {
            block_reduce_v[i * (BLOCK_SIZE / WARP_SIZE) + warp_id] = local_tmp_float;
        }
    }
    __syncthreads();
    // #------------------------------------------------------------------------
    ...
    *(uint4 * )(&local_half2[0]) = *(uint4 * )(&shared_HEAD_DIM0[(lane_id % 16) * 8]);
    ...
    for (int iter = tid; iter < SEQ_LEN / 8; iter += BLOCK_SIZE) {
        ...
        for (int d_iter = 0; d_iter < ATTN_SPLIT / (BLOCK_SIZE / WARP_SIZE * 8); d_iter += BLOCK_SIZE) {
            #pragma unroll
            ...
            if (lane_id == 0) {
                ...
                *(uint4 * )(&block_reduce_half[d_iter * (BLOCK_SIZE / WARP_SIZE * 8) + warp_id * 8]) = *(uint4 * )(&local_attn[0]);
            }
        }
    ...
    for (int iter = 0; iter < EMBEDDING_DIM / (WARP_SIZE * 8); ++iter) {
        __align__(16) half attn_value[8] = {
            __float2half(0.0), __float2half(0.0), __float2half(0.0), __float2half(0.0),
            __float2half(0.0), __float2half(0.0), __float2half(0.0), __float2half(0.0)
        };
    ...
    for (int i = 0; i < HEAD_NUM; ++i) {
        ...
        #pragma unroll
        for (int di = 0; di < EMBEDDING_TILE_LENGTH; ++di) {
            residual_embedding_reg[di] += weight_reg[di];
        } 
    }
    {
        float local_tmp_float = 0;
    ...
    if(tid < FFN_HIDDEN / COOPERATE_BLOCK_NUM) { // 4096 / 32 = 128
        ...
        #pragma unroll
        for (int di = 0; di < BLOCK_SIZE / WARP_SIZE / 8; di++) {
            *(uint4 * )(&local_half2[8 * di]) = *(uint4 * )(&block_reduce_half[(FFN_HIDDEN / COOPERATE_BLOCK_NUM + tid) * (BLOCK_SIZE / WARP_SIZE) + 8 * di]);
        }
        half local1 = 0, local2 = 0;
        #pragma unroll
        for (int di = 0; di < BLOCK_SIZE / WARP_SIZE; di++) {
    ...
    // ffn 2
    #pragma unroll
    for (int i = 0; i < EMBEDDING_TILE_LENGTH; ++i) {
    ...
    for (int i = 0; i < (FFN_HIDDEN / COOPERATE_BLOCK_NUM); ++i) {
        ...
        for (int di = 0; di < EMBEDDING_TILE_LENGTH / 8; di++) {
            *(uint4 * )(&weight_reg[8 * di]) = *(uint4 * )(&ffn_2[
                (head_idx * (FFN_HIDDEN / COOPERATE_BLOCK_NUM) + i) * EMBEDDING_DIM + 16 * tid + 8 * di
            ]);
        }
        #pragma unroll
        for (int di = 0; di < EMBEDDING_TILE_LENGTH; ++di) {
            embedding_reg[di] += __hmul(ffn_value, weight_reg[di]);
        }
    }
    #pragma unroll
    for (int di = 0; di < EMBEDDING_TILE_LENGTH  / 8; ++di) {
        *(uint4 * )(&global_reduction[batch_idx * HEAD_NUM * EMBEDDING_DIM + head_idx * EMBEDDING_DIM + tid * 16 + 8 * di]) = *(uint4 * )(&embedding_reg[8 * di]);
    }
    grid.sync();
    // #------------------------------------------------------------------------
    ...
    if (head_idx == 0) {
        ...
        #pragma unroll
        for (int di = 0; di < EMBEDDING_TILE_LENGTH  / 8; ++di) {
            *(uint4 * )(&output[batch_idx * EMBEDDING_DIM + tid * 16 + 8 * di]) = *(uint4 * )(&residual_embedding_reg[8 * di]);
        }
    }
...
int main(int argc, char **argv) {
    ...
    size_t dynamicShMemSize = sizeof(float) * (BLOCK_SIZE / WARP_SIZE) + sizeof(half) * (BLOCK_REDUCE_HALF + HEAD_DIM * (2 + BLOCK_SIZE / WARP_SIZE));
