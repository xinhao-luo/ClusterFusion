...
) 
{
    ...
    uint32_t max_shmem_size = ((((DIM_PER_BLOCK * sizeof(half) + DIM_BLOCK_REDUCE * sizeof(float) + 127) & ~127) +  2 * TMA_LOAD_ONCE * MAX_SMEM_DIM * sizeof(half) + 127) & ~127) + 3 * HEAD_DIM * sizeof(half);
