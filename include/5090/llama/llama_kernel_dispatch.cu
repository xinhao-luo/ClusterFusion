...
) 
{
    ...
    uint32_t max_shmem_size = ((((DIM_PER_BLOCK * sizeof(half) + 2 * NUM_PER_ROW_2 * sizeof(float) + 127) & ~127) +  2 * TMA_LOAD_ONCE * MAX_SMEM_DIM * sizeof(half) + 127) & ~127) + 3 * HEAD_DIM * sizeof(half);
