...
def generate_ninja_build_for_op(
    ...
    system_includes = [
        ...
        "$cuda_home/include",
        jit_env.CLUSTERFUSION_INCLUDE_DIR.resolve(),
        jit_env.CLUSTERFUSION_CSRC_DIR.resolve(),
    ]
    ...
    env_extra_ldflags = os.environ.get("CLUSTERFUSION_EXTRA_LDFLAGS")
    if env_extra_ldflags:
        try:
            ...
            print(
                f"Warning: Could not parse CLUSTERFUSION_EXTRA_LDFLAGS with shlex: {e}. Falling back to simple split.",
                file=sys.stderr,
            )
