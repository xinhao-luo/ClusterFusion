...
// nvcc --generate-code=arch=compute_90a,code=sm_90a -O3 -std=c++17 -lcuda tma_gemv.cu -o test && ./test
...
)
{
    ...
    for (int id = 0; id < DIM_PER_BLOCK / TMA_LOAD_ONCE; id++) {
        if (tid == 0) {
            ...
            token = cuda::device::barrier_arrive_tx(bar, 1, sizeof(weight_qkv));
        } else {
    ...
    //         } else {
    //             *(uint4*)(&reg_weight[0]) = *(uint4*)(&k_cache[kv_idx + i * HIDDEN_DIM + j]);
    //         }
    //         #pragma unroll
    //         for (int d = 0; d < NUM_PER_THREAD; d++) {
    ...
    // tmp = 0.0;
    // if (tid == 0) {
    //     cde::cp_async_bulk_tensor_2d_global_to_shared(&weight_qkv, &tensor_map, head_id * HEAD_DIM, HIDDEN_DIM * 2 + cluster_block_id * DIM_PER_BLOCK, bar);
    ...
    //     if (id % 2) {
    //         if (tid == 0) {
    //             cde::cp_async_bulk_tensor_2d_global_to_shared(&weight_qkv_buffer, &tensor_map, head_id * HEAD_DIM, HIDDEN_DIM * 2 + cluster_block_id * DIM_PER_BLOCK + id * TMA_LOAD_ONCE, bar_buffer);
    ...
    //     } else {
    //         if (tid == 0) {
    //             cde::cp_async_bulk_tensor_2d_global_to_shared(&weight_qkv, &tensor_map, head_id * HEAD_DIM, HIDDEN_DIM * 2 + cluster_block_id * DIM_PER_BLOCK + id * TMA_LOAD_ONCE, bar);
