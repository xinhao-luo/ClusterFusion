...
#include "dsm.cuh"
#include "mma.h"
// nvcc --generate-code=arch=compute_90a,code=sm_90a -O3 -std=c++17 -lcuda decode_ds_v2.cu -o test && ncu --metrics dram__bytes_read.sum,dram__bytes_write.sum ./test

// && ncu --metrics dram__bytes_read.sum,dram__bytes_write.sum ./test
...
__global__ void __cluster_dims__(CLUSTER_SIZE, 1, 1) DeepSeekDecoderLayerKernel(
    ...
    half* weight = (half*)((uintptr_t)(smem + DIM_PER_BLOCK * BATCH_SIZE * sizeof(half)) + 127 & ~127);
    half* local_qkv = (half*)((uintptr_t)(smem + DIM_PER_BLOCK * BATCH_SIZE * sizeof(half) + 2 * TMA_LOAD_ONCE * NOPE_HEAD_DIM * sizeof(half)) + 127 & ~127);
    float* reduction = (float*)((uintptr_t)(smem + DIM_PER_BLOCK * BATCH_SIZE * sizeof(half) + 2 * TMA_LOAD_ONCE * NOPE_HEAD_DIM * sizeof(half) + MLA_HEAD_DIM * 2 * BATCH_SIZE * sizeof(half)) + 127 & ~127);
    ...
    // DSM All-reduce
    size = BATCH_SIZE * MLA_HEAD_DIM * 2 * sizeof(half);
    ...
    // DSM All-gather
    size = BATCH_SIZE * 128 * sizeof(half);
    src_addr = static_cast<uint32_t>(__cvta_generic_to_shared(&local_qkv[cluster_block_id * BATCH_SIZE * 128]));
    dst_addr = static_cast<uint32_t>(__cvta_generic_to_shared(&local_qkv[cluster_block_id * BATCH_SIZE * 128]));
    ...
    for (int b = 0; b < BATCH_SIZE; b++) {
        ...
        *(uint4*)(&reg_input[0]) = *(uint4*)(&local_qkv[b * MLA_HEAD_DIM + input_idx_3]);
        block.sync();

        // Preload kv_cache
        if (tid == 0) {
            cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[0], &tensor_map_kv_cache, 0, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK, bar[0]);
            cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[TMA_LOAD_ONCE_NUM_ATTN / 2], &tensor_map_kv_cache_, KV_LORA_RANK / 2, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK, bar[0]);
        ...
        for (int id = 1; id < KV_DIM_PER_BLOCK / TMA_LOAD_ONCE_ATTN; id++) {
            if (tid == 0) {
                cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[(id % 2) * TMA_LOAD_ONCE_NUM_ATTN], &tensor_map_kv_cache, 0, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK + id * TMA_LOAD_ONCE_ATTN, bar[id % 2]);
                cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[(id % 2) * TMA_LOAD_ONCE_NUM_ATTN + TMA_LOAD_ONCE_NUM_ATTN / 2], &tensor_map_kv_cache_, KV_LORA_RANK / 2, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK + id * TMA_LOAD_ONCE_ATTN, bar[id % 2]);
        ...
        for (int j = 0; j < DEC_TILE; j++) {
            if (cluster_block_id == CLUSTER_SIZE - 1 && tile_row == 1 && j == DEC_TILE - 1)
                *(uint4*)(&reg_weight[0]) = *(uint4*)(&local_qkv[BATCH_SIZE * MLA_HEAD_DIM + b * MLA_HEAD_DIM + input_idx_3]);
            else
        ...
        *(uint4*)(&weight[tile_row * MLA_HEAD_DIM + tile_col * NUM_PER_THREAD]) = *(uint4*)(&reg_reduce[0]);
        ...
        if(tid < NUM_THREAD_PER_ROW_3) {
            *(uint4*)(&local_qkv[b * MLA_HEAD_DIM + tid * NUM_PER_THREAD]) = *(uint4*)(&reg_reduce[0]);
        }
        block.sync();
    }
    // DSM Ring All-reduce
    size = KV_LORA_RANK * BATCH_SIZE * sizeof(half);
    src_addr = static_cast<uint32_t>(__cvta_generic_to_shared(local_qkv));
    dst_addr = static_cast<uint32_t>(__cvta_generic_to_shared(weight));
    dsm_ring_allreduce<CLUSTER_SIZE, Stage::ATTN_DEEPSEEK>(
        size, tid, KV_LORA_RANK * BATCH_SIZE, cluster_block_id,  
        src_addr, dst_addr, bar_ptr, 
        neighbor_dst_bar, local_qkv, weight);
    ...
    size = BATCH_SIZE * NOPE_HEAD_DIM * sizeof(half);
    src_addr = static_cast<uint32_t>(__cvta_generic_to_shared(local_qkv));
    dst_addr = static_cast<uint32_t>(__cvta_generic_to_shared(weight));
    dsm_ring_allreduce<CLUSTER_SIZE, Stage::ATTN>(
        size, tid, BATCH_SIZE * NOPE_HEAD_DIM, cluster_block_id,  
        src_addr, dst_addr, bar_ptr, 
        neighbor_dst_bar, local_qkv, weight);
...
int main(int argc, char** argv) {
    cudaFuncSetAttribute(DeepSeekDecoderLayerKernel, cudaFuncAttributeNonPortableClusterSizeAllowed, 16);
    uint32_t max_shmem_size = DIM_PER_BLOCK * BATCH_SIZE * sizeof(half) + 2 * TMA_LOAD_ONCE * NOPE_HEAD_DIM * sizeof(half) + MLA_HEAD_DIM * 2 * BATCH_SIZE * sizeof(half) + NUM_WARPS * sizeof(float) + 127 & ~127;
    cudaFuncSetAttribute(DeepSeekDecoderLayerKernel, cudaFuncAttributeMaxDynamicSharedMemorySize, max_shmem_size);
    ...
    h_ckv_cache = new half[BATCH_SIZE * SEQ_LEN * MLA_HEAD_DIM];
    ...
    fill_matrix(h_ckv_cache, BATCH_SIZE * SEQ_LEN * MLA_HEAD_DIM);
    ...
    cudaMalloc(reinterpret_cast<void**>(&d_ckv_cache), sizeof(half) * BATCH_SIZE * SEQ_LEN * MLA_HEAD_DIM);
    ...
    cudaMemcpy(reinterpret_cast<void*>(d_ckv_cache), h_ckv_cache, sizeof(half) * BATCH_SIZE * SEQ_LEN * MLA_HEAD_DIM, cudaMemcpyHostToDevice);
    ...
    uint64_t size_kv_cache[rank] = {MLA_HEAD_DIM, BATCH_SIZE * SEQ_LEN};
    ...
    for (int i = 0; i < wmup; i++) {
        DeepSeekDecoderLayerKernel<<<grid, block, max_shmem_size>>>(
            d_output,
            d_input,
            tensor_map_weight_q,
