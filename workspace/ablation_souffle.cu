...
#include <math.h>
#include <fstream>
// nvcc --generate-code=arch=compute_90a,code=sm_90a -O3 -std=c++17 -lcuda ablation_souffle.cu -o test && ./test
...
__global__ void single_decode_kernel(
    ...
    half* global_reduce_qkv,    // hidden_dim  
    half* global_reduce_softmax,    // hidden_dim  
    half* global_reduce_attn,    // hidden_dim  
    ...
    for (int i = tid; i < HEAD_DIM; i+=BLOCK_SIZE) {
        atomicAdd(&global_reduce_qkv[head_idx*HEAD_DIM + i], q_shmem[i]);
        atomicAdd(&global_reduce_qkv[HIDDEN_DIM + head_idx*HEAD_DIM + i], kv_shmem[i]);
    }
    grid.sync();
    // #------------------------------------------------------------------------
    // # Q @ K^T
    ...
    if(tid < HEAD_DIM / 2) {
        *(__half2 * )(&q_shmem[tid * 2]) = *(__half2 * )(&global_reduce_qkv[head_idx * HEAD_DIM + tid * 2]);
    }
    // Q @ K^T header
    load_half_matrix_to_shared_mem(
    ...
    if (tid == 0) {
        atomicAdd(&global_reduce_softmax[head_idx], local_sum);
    }
    grid.sync();
    if (tid == 0) {
        block_reduce_float[0] = global_reduce_softmax[head_idx];
    }
    block.sync();
    ...
    // }
    for(int i = tid; i < HEAD_DIM; i+=BLOCK_SIZE) {
        atomicAdd(&global_reduce_qkv[2 * HIDDEN_DIM + head_idx * HEAD_DIM + i], kv_shmem[i]);
    }
    grid.sync();

    for(int i = tid; i < HEAD_DIM; i+=BLOCK_SIZE) {
        kv_shmem[i] = global_reduce_qkv[2 * HIDDEN_DIM + head_idx * HEAD_DIM + i];
    }
    block.sync();
    ...
    for(int i = tid; i < HEAD_DIM; i+=BLOCK_SIZE) {
        atomicAdd(&global_reduce_attn[head_idx * HEAD_DIM + i], attention_output_shmem[i]);
    }
    grid.sync();

    for(int i = tid; i < HEAD_DIM; i+=BLOCK_SIZE) {
        attention_output_shmem[i] = global_reduce_attn[head_idx * HEAD_DIM + i];
    }
    block.sync();
    // # ~~~ global_reduction ~~~
...
int main(int argc, char **argv) { 
    ...
    cudaMalloc(reinterpret_cast<void**>(&global_reduce_softmax), sizeof(float) * HEAD_NUM);
    ...
    void *kernelArgs[] = {
        &d_output, &d_input,
        &global_reduce_qkv, &global_reduce_softmax, &global_reduce_attn, &global_reduce,
