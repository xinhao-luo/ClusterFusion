...
// nvcc --generate-code=arch=compute_90a,code=sm_90a -O3 -std=c++17 -lcuda linear-attn_v2.cu -o test && ncu --metrics dram__bytes_read.sum,dram__bytes_write.sum ./test
...
__global__ void __cluster_dims__(CLUSTER_SIZE, 1, 1) LlamaDecoderLayerKernel(
    ...
    half* local_qkv = (half*)((uintptr_t)(smem + DIM_PER_BLOCK * BATCH_SIZE * sizeof(half) + 2 * TMA_LOAD_ONCE * HEAD_DIM * sizeof(half)) + 127 & ~127);
    // (half*)(weight + 2 * TMA_LOAD_ONCE * HEAD_DIM * sizeof(half));
    float* reduction = (float*)((uintptr_t)(smem + DIM_PER_BLOCK * BATCH_SIZE * sizeof(half) + 2 * TMA_LOAD_ONCE * HEAD_DIM * sizeof(half) + 3 * HEAD_DIM * BATCH_SIZE * sizeof(half)) + 127 & ~127);
    // (local_qkv + 3 * HEAD_DIM * BATCH_SIZE * sizeof(half));

    // Init shared memory
    // __shared__ __align__(16) half input_shmem[DIM_PER_BLOCK * BATCH_SIZE];
    // __shared__ alignas(128) half weight[2 * TMA_LOAD_ONCE * HEAD_DIM];
    // __shared__ __align__(16) half local_qkv[3 * HEAD_DIM * BATCH_SIZE];
    ...
    // DSM Ring All-reduce
    size = (HEAD_DIM * BATCH_SIZE) * sizeof(half);
    ...
    dsm_ring_allreduce<CLUSTER_SIZE, Stage::LINEAR>(
        size, tid, HEAD_DIM * BATCH_SIZE, cluster_block_id,  
    ...
    for (int b = 0; b < BATCH_SIZE; b++) {
        ...
        *(uint4*)(&reg_input[0]) = *(uint4*)(&local_qkv[b * HEAD_DIM + input_idx_2]);
        block.sync();

        // Preload kv_cache
        if (tid == 0) {
            cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[0], &tensor_map_k_cache, cluster_head_idx, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK, bar[0]);
            token[0] = cuda::device::barrier_arrive_tx(bar[0], 1, TMA_LOAD_ONCE_SIZE_ATTN);
            cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[TMA_LOAD_ONCE_NUM_ATTN], &tensor_map_v_cache, cluster_head_idx, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK, bar[2]);
        ...
        for (int id = 1; id < KV_DIM_PER_BLOCK / TMA_LOAD_ONCE_ATTN; id++) {
            if (tid == 0) {
                cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[(id % 2) * TMA_LOAD_ONCE_NUM], &tensor_map_k_cache, cluster_head_idx, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK + id * TMA_LOAD_ONCE_ATTN, bar[id % 2]);
            ...
            if (tid == 0) {
                cde::cp_async_bulk_tensor_2d_global_to_shared(&weight[(id % 2) * TMA_LOAD_ONCE_NUM + TMA_LOAD_ONCE_NUM_ATTN], &tensor_map_v_cache, cluster_head_idx, b * SEQ_LEN + cluster_block_id * KV_DIM_PER_BLOCK + id * TMA_LOAD_ONCE_ATTN, bar[2 + id % 2]);
        ...
        for (int j = 0; j < DEC_TILE; j++) {
            if (cluster_block_id == CLUSTER_SIZE - 1 && warp_id == NUM_WARPS - 1 && lane_id / NUM_THREAD_PER_ROW_2 == 1 && j == DEC_TILE - 1)
                *(uint4*)(&reg_weight[0]) = *(uint4*)(&local_qkv[BATCH_SIZE * HEAD_DIM + b * HEAD_DIM + input_idx_2]);
            else
        ...
        for (int j = 0; j < DEC_TILE; j++) {
            if (cluster_block_id == CLUSTER_SIZE - 1 && warp_id == NUM_WARPS - 1 && lane_id / NUM_THREAD_PER_ROW_2 == 1 && j == DEC_TILE - 1) 
                *(uint4*)(&reg_weight[0]) = *(uint4*)(&local_qkv[BATCH_SIZE * 2 * HEAD_DIM + b * HEAD_DIM + input_idx_2]);
            else
        ...
        *(uint4*)(&weight[tile_row * HEAD_DIM + tile_col * NUM_PER_THREAD]) = *(uint4*)(&reg_reduce[0]);
        ...
        if(tid < NUM_THREAD_PER_ROW_2) {
            *(uint4*)(&local_qkv[b * HEAD_DIM + tid * NUM_PER_THREAD]) = *(uint4*)(&reg_reduce[0]);
        }
        block.sync();
    }
...
int main(int argc, char** argv) {   
    ...
    // cudaDeviceGetAttribute(&max_dynamic_smem, cudaDevAttrMaxSharedMemoryPerBlockOptin, device);

    // printf("Max dynamic shared memory per block (opt-in): %d bytes\n", max_dynamic_smem);

    cudaFuncSetAttribute(LlamaDecoderLayerKernel, cudaFuncAttributeNonPortableClusterSizeAllowed, 16);
    // size_t input_bytes = DIM_PER_BLOCK * BATCH_SIZE * sizeof(half);
    // size_t weight_bytes = 2 * TMA_LOAD_ONCE * HEAD_DIM * sizeof(half);
    // size_t qkv_bytes = 3 * HEAD_DIM * BATCH_SIZE * sizeof(half);
    // size_t reduction_bytes = 2 * NUM_PER_ROW_2 * sizeof(float);
    // size_t cluster_float_bytes = 2 * sizeof(float); // for cluster_local_sum & max
    // size_t weight_offset = input_bytes;
    // size_t local_qkv_offset = weight_offset + weight_bytes;
    // size_t reduction_offset = local_qkv_offset + qkv_bytes;

    // size_t max_shmem_size = reduction_offset + reduction_bytes + 127 & ~127;
    uint32_t max_shmem_size = DIM_PER_BLOCK * BATCH_SIZE * sizeof(half) + 2 * TMA_LOAD_ONCE * HEAD_DIM * sizeof(half) + 3 * HEAD_DIM * BATCH_SIZE * sizeof(half) + 2 * NUM_PER_ROW_2 * sizeof(float) + 127 & ~127;
    ...
    uint64_t size_k_cache[rank] = {HIDDEN_DIM, BATCH_SIZE * SEQ_LEN};
    ...
    uint64_t size_v_cache[rank] = {HIDDEN_DIM, BATCH_SIZE * SEQ_LEN};
